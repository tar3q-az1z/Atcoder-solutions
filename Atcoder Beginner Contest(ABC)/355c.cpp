...
int main(void){
	...
	while(++tcase, t--){
		...
		x = 1;
		for(int i = 1; i <= N; ++i){
			for(int j = 1; j <= N; ++j) m1[i].insert(x + j - 1);
			x = i * N + 1;
		}
		x = 1;
		for(int i = 1; i <= N; ++i){
			for(int j = 1; j <= N; ++j, x += N) m2[i].insert(x);
			x = i + 1;
		}
		int cnt = 0, ok = 0;
		for(int i = 0; i < T; ++i){
			int ai; 
			cin >> ai; 
			if(sz(cr1)) cr1.erase(ai);
			if(sz(cr2)) cr2.erase(ai);
			int r, c;
			if(ai % N == 0 && sz(m1) && sz(m2)){
				r = ai / N, c = N;
				m1[r].erase(ai);
				m2[c].erase(ai);
			}else if(sz(m1) && sz(m2)){
			...
			++cnt;
			if(sz(cr1) == 0 || sz(cr2) == 0 || sz(m1[r]) == 0 || sz(m2[c]) == 0){
				ok = 1;
				break;
			}
		}
